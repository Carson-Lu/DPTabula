...
def is_domain_partially_in_domain(v1, v0): 
    ...
    # domain v1 corners (TL = top left, TR = top right, BL = bottom left, BR = bottom right)
    is_partially_in = False
    
    v0_L, v0_R = v0[0], v0[1]
    v1_L, v1_R = v1[0], v1[1]
    
    if is_in_domain(v1_L, v0_L, v0_R):
...
class privtree():
    ...
    def tree_main(self, x, lam, delta):
        ...
        while len(unvisited_domains) > 0: # while unvisited_domains is not empty
            for unvisited in unvisited_domains[:]:  # Create a copy to avoid modification during iteration
        ...
        # Ensure we always have at least one domain
        if len(domains) == 0:
            # If no domains were created, use the original range
            domains = [v0]
    ...
    def inverse_data(self, data_col, domains):
        # Handle the case where data contains -1 values (default from transform_data)
        # For -1 values, we'll use the first domain as a fallback
        result = []
        for i in data_col:
            if int(i) == -1 or int(i) >= len(domains):
                # Use the first domain as fallback for invalid indices
                if len(domains) > 0:
                    result.append(np.random.uniform(domains[0][0], domains[0][1]))
                else:
                    # If no domains exist, use a reasonable default
                    result.append(0.0)
            else:
                result.append(np.random.uniform(domains[int(i)][0], domains[int(i)][1]))
