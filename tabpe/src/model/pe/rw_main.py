...
def vote(public, private, count, noise_multiplier, columns, info):
    ...
    for epoch in tqdm(range(max(e, 1), 1 + args.epochs), desc="Epochs"):
        ...
        for label in public:
            ...
            if epoch <= args.sampling_epochs:
                ...
                label_embeddings = get_embeddings(label_public, columns, info)
                private_embeddings = get_embeddings(label_private, columns, info)
